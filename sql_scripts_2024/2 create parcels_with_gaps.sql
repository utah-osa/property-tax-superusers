...
-- DROP TABLE IF EXISTS research_public.parcels_with_gaps;

-- CREATE OR REPLACE TABLE research_public.parcels_with_gaps 
-- (county STRING,
--  clean_id STRING, 
--  year_impacted INT64);
...
-- CREATE OR REPLACE PROCEDURE research_public.find_parcel_gaps_for_county_year(county_name STRING, year_check INT64)
-- BEGIN
...
--   INSERT INTO research_public.parcels_with_gaps 

--   SELECT county, parcel_id, year_test_p AS year_impacted
--   FROM `ut-sao-tax-prod.research_public.tax_roll`

--   WHERE parcel_id IN (SELECT parcel_id FROM `ut-sao-tax-prod.research_public.tax_roll` 
--                       WHERE county = county_p AND year = year_test_p+1) 

--     AND parcel_id IN (SELECT parcel_id FROM `ut-sao-tax-prod.research_public.tax_roll` 
--                       WHERE county = county_p AND year = year_test_p-1) 

--     AND parcel_id NOT IN (SELECT parcel_id FROM `ut-sao-tax-prod.research_public.tax_roll` 
...
-- CALL research_public.find_parcel_gaps_for_county_year("Salt Lake County", 2022)
...
SET years_to_check =  [2019, 2020, 2021, 2022, 2023];
...
DO
  ...
  IF i > ARRAY_LENGTH(years_to_check) THEN 
    LEAVE; 
  END IF;
  CALL research_public.find_parcel_gaps_for_county_year(county_name_to_check.county, years_to_check[ORDINAL(i)]);
  END LOOP; 
END FOR;
